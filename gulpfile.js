...
function server() {
  return src("src/server/**/*").pipe(gulpCopy(BUILD_APP_DIR, { prefix: 2 }));
}
...
function font() {
  return src("src/client/font/*").pipe(
    gulpCopy(path.join(BUILD_PUBLIC_DIR, "font"), { prefix: 3 })
  );
}
